  ...
  <header class="sticky top-0 z-40 w-full transition-colors duration-300" :class="scrolled ? 'bg-canvas/95 backdrop-blur border-b border-hairline' : 'bg-canvas border-b border-transparent'">
    ...
    <div class="container-bmw flex h-16 items-center justify-between gap-md">
      ...
      <nav class="hidden items-center gap-xl lg:flex" aria-label="Primary">
        <NuxtLink v-for="link in NAV_LINKS" :key="link.to" :to="link.to" class="text-nav-link text-body transition-colors hover:text-ink" active-class="text-ink">{{ link.label }}</NuxtLink>
      </nav>

      <!-- Right: search + cart -->
      <div class="flex items-center gap-xs">
        <button class="flex h-10 w-10 items-center justify-center text-ink hover:text-body-strong" aria-label="Search" @click="ui.openSearch()">
        ...
        <button class="relative flex h-10 w-10 items-center justify-center text-ink hover:text-body-strong" aria-label="Open cart" @click="ui.openCart()">
